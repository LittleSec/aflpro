...
int send_mutated_message(void)
{
    if (N_results != NULL)
    {
         ...
         * target process */
            ...
            {
                if (sended == -1)
                {                            // if sendfile didn't work, use read() and write()
                    lseek(fd, 0, SEEK_SET); // reset to the beginning of the file
                    // create a temporary buffer to hold all the data in the file
                    u8 tempbuf[statbuf.st_size];
...
*/
            ...
            int close_result;
            if ((close_result = close(N_fd)) != 0)
                FATAL("client socket close failed: %d", close_result);
        }
        else if (N_results->ai_socktype == SOCK_DGRAM)
        {
            /* UDP datagrams are used.
...
{
    if (child_pid <= 0)
        FATAL("child_pid process is stopped");
    ...
    */
    ...
    if (memory_cost > max_pid_memcost)
        max_pid_memcost = memory_cost;
    if (memory_cost < min_pid_memcost)
        min_pid_memcost = memory_cost;
...
int match_format(char *msg)
{
    ...
    for (int i = 0; i < tempNode->FieldNum; i++)
    {
        ...
        if (strcmp(field_type, "delimiter") == 0)
        {
            ...
            if (strcmp(msg_test, delim_value) != 0)
            {              //compare if the front-length string is equal to default value
                return 0; //no, return;
            }
            else
            {
        ...
        if (strcmp(field_type, "string") == 0)
        {
            ...
            if (str_constraint == 0)
                FATAL("constraint for string can not be 0");
...
static u8 write_to_testcase_with_grammar(u8 *mem, u32 len)
{
    ...
    if (result == 0)
    { // if the format is not matched, return 0;
        return 0;
    }
...
#define SP20 SP10 SP10
    ...
    else
    {
        ...
        if (queue_cycle == 1)
            ...
            if (cycles_wo_finds < 25)
            strcpy(tmp, cYEL);
        else

            /* No finds for a long time and no test cases to try. */
            if (cycles_wo_finds > 100 && !pending_not_fuzzed)
            strcpy(tmp, cLGN);
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:G:L:s")) > 0)
         ...
         */
            ...
            if (G_option_specified)
                FATAL("multiple -G options not allowed");
        ...
        case 's': //stateful
            ...
            if (N_stateful_specified)
                FATAL("multiple -s options not allowed");
